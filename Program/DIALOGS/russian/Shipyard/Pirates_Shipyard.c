...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    if ( sti(Pchar.Ship.Type) != SHIP_NOTUSED)
    {
	    ...
	    int  capacityMatherial1 = makeint(shipCapacity /(shipClass*2));
	    int	 capacityMatherial2 = makeint(cannonQ *10 /shipClass);
	    int  capacityWorkPrice  = makeint(shipCapacity/5 * 200);
	...
	switch (Dialog.CurrentNode)
	{
		...
		case "ship_tunning_Capacity":
			...
			s1 = s1 + DLG_TEXT_SHY[105]+ capacityMatherial1 + DLG_TEXT_SHY[106]+ capacityMatherial2+DLG_TEXT_SHY[107];
			s1 = s1 + DLG_TEXT_SHY[108] + capacityWorkPrice + DLG_TEXT_SHY[109];
            dialog.Text = s1;
